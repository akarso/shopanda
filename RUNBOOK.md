...
- **Progress is visible the same way any other job's is**: the underlying `search.reindex` job shows up in `jobs:list`/`jobs:show`/the jobs admin API (PR-1028/1029/1032) like any other job — a stuck or failed reindex is retried/investigated the same way, not through a separate mechanism.
