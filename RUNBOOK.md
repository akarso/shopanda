...
- If the worker process crashes mid-run, the `search_index_runs` row is left `processing` with a partial `processed_count` — it is not automatically resumed or retried. The underlying job itself still gets the queue's normal retry/failure handling; reconcile the run row's status with the job's own outcome (`jobs:show <job-id>`) if a run looks stuck.
