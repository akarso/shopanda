...
- This PR ships full-table reindex only — the same scope `search:reindex` always did — just running through the queue instead of the CLI process. Partial/scoped reindex (explicit product/category IDs, "changed since") is a planned follow-up (PR-1034), not available yet.
