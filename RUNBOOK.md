...
- **The index no longer goes stale between reindexes (PR-1036).** Editing a product, its price, its stock, or its category assignment through the admin API automatically enqueues a scoped reindex for that product — event-driven, debounced (`internal/application/search.IndexUpdateSubscriber`, a few seconds per product, so a burst of edits to one product enqueues one reindex, not one per edit), and going through the same job queue/`search_index_runs` tracking as every other reindex, not a separate ad-hoc mechanism. This covers changes made through the admin HTTP API; anything that mutates product/price/stock/category-assignment data through a different path (a plugin writing directly to the database, the bulk ERP stock-sync importer) won't publish the underlying event and so won't trigger this automatically — `search:reindex`/the admin API's manual/scheduled paths remain the fallback for whatever the event coverage doesn't reach. See `docs/guides/DEVELOPER.md`'s "Add Custom Event Listeners" section for the exact events a plugin author needs to publish to participate.
