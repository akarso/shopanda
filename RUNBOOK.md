...
- **`jobs:list`/`jobs:show`/the jobs admin API (PR-1028/1029/1032) show the underlying `search.reindex` job's queue status only** (pending/processing/done/failed, attempts) — a stuck or failed reindex is retried/investigated through that same job admin surface as any other job. They do **not** show reindex-specific progress: `search_index_runs.processed_count`/`total_count` isn't exposed there. `search:reindex --wait` prints the final indexed/total count once the run reaches a terminal state (not live incremental progress); a dedicated progress endpoint over `search_index_runs` is a planned follow-up (PR-1035).
- **`search:reindex` itself always triggers a full-table scan** — the same scope it always did. Partial/scoped reindex (explicit product/category IDs, or "changed since") exists as an application-layer capability as of PR-1034 (`ReindexService.Trigger` accepts `search.ScopeProducts`/`ScopeCategories`/`ScopeSince`, not just `ScopeAll`) but has no CLI flag or admin API surfacing it yet — that's PR-1035. A scope covering more than `search.reindex_full_scan_threshold` (default 20%, config-overridable) of the catalog is automatically substituted with a full scan by `Trigger` itself before enqueueing — visible, not silent: the run's recorded `scope_params` still shows what was requested, even though there's no UI to see that yet.
