...
- **This coverage has known gaps — the highest-traffic one is customer checkout.** `inventory.EventStockUpdated` is published only from the admin inventory "Adjust" endpoint. It is **not** published from: checkout reservation/release (`internal/application/checkout.ReserveInventoryStep` decrements/restores `stock.quantity` via raw SQL in `postgres.ReservationRepo`, not through `StockRepository`, and has no event bus wired at all — this means an ordinary customer purchase, the single highest-frequency stock-changing event in the system, does not keep the index fresh today); the reservation-expiry sweep (`ReleaseExpiredBefore` only returns a count, not which variants changed, so there's nothing to publish from without first widening that interface); returns restocking (`internal/application/returns.Service.Receive` — has a bus already, but no way to resolve a variant to its product ID yet); or either bulk stock path (`import:stock` CLI/CSV, or the ERP `StockSyncService.UpsertBySKU` API sync — both process many variants with no bus wired, so wiring one naively would enqueue one reindex per row instead of one per import). None of this is silent-by-oversight: each is called out explicitly as a known, accepted gap pending its own follow-up (checkout is the priority), not something to "just wire up" without also addressing the fan-out risk the bulk paths would create. `search:reindex`/the admin API's manual/scheduled paths remain the operational fallback for all of these until they're closed.
