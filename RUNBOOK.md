...
- If the worker process crashes mid-run, the `search_index_runs` row is left `processing` with a partial `processed_count` — it is not automatically resumed or retried. **A reconciliation sweep now runs automatically** (`search.reindex.reconcile`, cron `*/30 * * * *`, PR-1048): every 30 minutes it checks every run still `processing` after 2 hours (`ReconcileHandler`'s `staleAfter`) against its underlying `search.reindex` job (found via `payload->>'run_id'`, `jobs:show <job-id>`). If that job has already reached a real terminal status (`done`/`failed`/`cancelled`) — the terminal-`Finish`-write-exhausted-retries case, and the case where a job somehow reached a terminal status through some future path without ever touching the run — the sweep corrects the run to `failed`, recording why in `last_error`. It deliberately does **not** touch a run whose job still shows `pending`/`processing` (or is missing outright): job status alone can't distinguish a genuinely orphaned job (the crashed-worker case — there is no lease/heartbeat on the `jobs` table, so a job a worker was handling when it died stays `processing` forever too, with nothing to ever reclaim it) from one still legitimately running or retrying. Run `schedule:trigger search.reindex.reconcile` to fire the sweep immediately instead of waiting for the next tick.
- **For the ambiguous case the sweep leaves alone** (the run's underlying job is *also* still `processing`/`pending`, or missing) — first confirm independently that the job is never coming back (e.g. the worker process that held it is confirmed dead/restarted, and no reindex is actually progressing), then run `app search:reindex-runs:reconcile <run-id> --reason="<why>"` to flip that one run to `failed` with your reason recorded in `last_error`. It refuses (409) if the run isn't currently `processing`, and requires `--reason`. This is the supported alternative to editing `search_index_runs` directly in the database.
