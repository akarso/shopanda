...
- **A run can be left `processing` forever with no automatic way back**, in two cases: (1) the worker process crashes mid-run, or (2) the database is unavailable for the entire retry window of the terminal status write on the job's very last attempt (a handful of retries with backoff, longest on that last attempt specifically — see `ReindexHandler.failIfTerminal`'s doc comment) — in both cases the underlying job itself still gets the queue's normal retry/failure handling (`jobs:list --type=search.reindex` / `jobs:show <job-id>` shows its real outcome), but nothing ever calls `Finish` for the run row again once the job is done. There is no CLI or admin mutation yet to fix a run's status directly (that's a real gap, not an oversight) — reconciling it today means checking the job's actual outcome via `jobs:show <job-id>` and, if it's truly done while the run is still `processing`, correcting the `search_index_runs` row directly in the database.
