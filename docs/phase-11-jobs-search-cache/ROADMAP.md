...
| PR-1035 | Reindex admin API + progress | **Done.** `POST /admin/search/reindex` (`{"scope": "all"}` \| `{"scope": "products", "ids": [...]}` \| `{"scope": "categories", "ids": [...]}` \| `{"scope": "since", "since": "<RFC3339>"}`), `GET /admin/search/reindex/{runID}` for progress. A single explicit *product* ID skips the queue and calls `IndexProduct` synchronously, returning immediately. A single *category* ID does not get this treatment — `IndexCategory` now exists (PR-1037) but wiring the single-category sync shortcut was deliberately deferred, not bundled opportunistically into that PR — and is still enqueued the same as multiple IDs; see "Design notes" below. |
| PR-1036 | On-save incremental indexing | **Done.** `internal/application/search.IndexUpdateSubscriber` (same shape as `internal/application/cache/invalidation.go`) subscribes to `catalog.EventProductCreated`/`EventProductUpdated`, `pricing.EventPriceUpserted`, and a new `inventory.EventStockUpdated`, debouncing (5s per product ID) before calling `ReindexService.Trigger(ScopeProducts{...})` — queued, not synchronous. Replaces the ad-hoc synchronous `searchEngine.IndexProduct` calls `wire_services.go` used to make inline on product events (no queue, no retry, no debounce). Category-assignment changes reuse `catalog.EventProductUpdated` rather than a new event — see "Design notes" below. This is the "on save" mode; PR-1030's schedule-trigger and PR-1035's manual trigger are the other two. |
| PR-1037 | Category indexing + relationship fix | **Done.** Added `IndexCategory`/`RemoveCategory` to the `SearchEngine` port (Postgres: a `search_vector` column on `categories`, migration `072`; Meilisearch: a second `categories` index). Fixed `search.Product.CategoryID` (singular) → `CategoryIDs []string` — the catalog domain already supports many-to-many product↔category assignment (`product_categories` junction table, `AssignCategory`/`RemoveCategory`/`ListCategoryIDsByProduct`); the search index had silently been unable to represent a product in more than one category since day one, though this only actually bit the Meilisearch backend in practice (see "Design notes" below). `internal/application/search.IndexUpdateSubscriber` gained `HandleCategoryCreated`/`Updated`/`Deleted`, wired to the category admin API's existing events. |
| PR-1038 | Search admin GUI | **Done.** Reindex trigger with a scope picker (all / products / categories / since-date), a new polling progress bar (no existing pattern to reuse — Track A's job list/detail screens don't poll either), a run history table (backed by a new `RunStore.List`/`GET /admin/search/reindex`, since none existed), and a per-row "reindex now" action on the product/category admin grids — which also required finally wiring `scope=categories`' single-ID case to `SearchEngine.IndexCategory` synchronously, closing the gap PR-1037 deliberately deferred (see "Design notes: PR-1035 shipped ahead of PR-1037" below, now resolved). Track B complete. |
...
PR-1035's original scope described the single-item synchronous path as calling `IndexProduct`/`IndexCategory` symmetrically for `scope=products`/`scope=categories`. `IndexCategory` didn't exist yet at the time — it was PR-1037, tracked separately (and non-trivial: it also fixed `Product.CategoryID` → `CategoryIDs`, a real behavior change, not just an added method). Rather than block the admin API and progress endpoint — the actual deliverable this phase set out to build — on a category-indexing PR with its own independent scope, PR-1035 shipped with `scope=categories` always going through the queued/bulk path (`ReindexService.Trigger`, which already resolves category scope to product IDs via `ProductIDsByCategory` and indexes those products — this works regardless of PR-1037), regardless of ID count. Only `scope=products` with exactly one ID gets the synchronous "reindex this now" treatment.

`IndexCategory` now exists (PR-1037 shipped), but `search_admin.go`'s single-category synchronous shortcut was deliberately **not** added as part of that PR — it wasn't in PR-1037's own listed Scope (only mentioned informally in this note), and adding it would have meant more constructor-signature churn on `SearchAdminHandler` for a convenience path, not a correctness fix.

**Resolved by PR-1038**: that PR's own spec required the category admin grid's row action to call "the same synchronous single-item endpoint" as the product grid — which made this gap a genuine prerequisite rather than an optional nice-to-have, so PR-1038 wired it: `scope=categories` with exactly one ID now takes the synchronous path (`SearchEngine.IndexCategory` directly), mirroring `scope=products`' existing shortcut exactly. See PR-1038.md's own "Design decisions."

### Design notes: PR-1036's category-assignment event and the removed inline sync handlers

PR-1036's own plan named two open questions to confirm during implementation:

- **Category-assignment change events reuse `catalog.EventProductUpdated`, not a new event.** `internal/interfaces/http/admin/category_product_assignment_admin.go`'s `Assign`/`Unassign` didn't publish anything at all before this PR. Rather than add a new `catalog.EventCategoryAssignmentChanged` (or similar) that every future consumer would also need to remember to subscribe to, `Assign`/`Unassign` publish `catalog.EventProductUpdated` with the product's real `ProductID`/`Name`/`Slug`/`Status` (the already-fetched product from `validateAssignmentTargets`, not a bare ID) — **not** left zero-valued as a first pass through this code briefly assumed. `rewrite.Subscriber` also subscribes to this event, synchronously, and builds a URL rewrite straight from `data.Slug` with no fallback lookup; a blank `Slug` would have registered `/` itself as the product's rewrite (or hit a path conflict if `/` was already claimed), and either way aborted the publish before the async cache-invalidation/search-reindex handlers ever ran (a sync handler error short-circuits `Publish`). A useful side effect of the reuse: `cache.InvalidationSubscriber` already subscribes to this event, so a category-assignment change now correctly invalidates the product's cache too, not just its search index.
- **Stock-adjustment events (`inventory.EventStockUpdated`) are wired from the interactive admin `PUT /api/v1/admin/inventory/{variantId}` path only, not from the bulk ERP stock-sync importer** (`internal/application/inventory.StockSyncService.UpsertBySKU`). That importer is constructed from several different process entry points (`serve`, `worker`, CLI schedule-trigger) via `wireIntegrationStockSyncer`, some of which don't have an `event.Bus` readily available at that point in their own setup — threading one through would have meant touching call sites unrelated to this PR's actual scope. A bulk stock import not participating in on-save indexing is an accepted gap for now (the existing manual/scheduled `search:reindex` remains the fallback for it); revisit if this turns out to matter operationally.

Also: the two existing inline `bus.OnAsync(catalog.EventProductCreated/EventProductUpdated, ...)` handlers in `wire_services.go` (which called `searchEngine.IndexProduct` directly, synchronously relative to the event, bypassing the job queue and `search_index_runs` tracking entirely) were **removed**, not left alongside the new subscriber — keeping both would have double-indexed every product edit (once immediately via the old path, once again a few seconds later via the new debounced job) and cluttered `search_index_runs` with a one-row entry per ordinary edit. This matches PR-1036's own "Out Of Scope" framing: synchronous on-save indexing is explicitly what this PR replaces with the async, queued, debounced approach every other side-effect-on-event in this codebase already uses.

### Design notes: PR-1037 — the relationship bug only ever bit Meilisearch

PR-1037.md's "Why" described the dropped-category-past-the-first bug as affecting "category-filtered search results... for every such [multi-category] product." Implementing the fix surfaced a narrower reality: the **Postgres** `SearchEngine` never stored category data on the indexed product document at all — `IndexProduct`/`RemoveProduct` only ever touch `products.search_vector` (derived from name/description), and `Search()`'s category filtering/faceting is a live `JOIN` against `product_categories`/`categories` at query time. So the Postgres backend was never actually affected by the singular `CategoryID` field's limitation; it was simply an unused field on `search.Product` from Postgres's point of view. The bug was real, but concentrated entirely in the **Meilisearch** adapter, which does store category data on the product document (`category_id` → now `category_ids`) and filters against it directly.

This changed the migration story: PR-1037.md's planned "migration test" (seed under the old schema, run a migration, confirm the new `CategoryIDs` reflects the full assignment set) doesn't apply on the Postgres side — there is no persisted `category_id` column to migrate, since none ever existed. The only genuinely new Postgres schema change is migration `072`, which adds a `search_vector` column (+ GIN index + trigger) to `categories` itself, mirroring `016_add_search_vector.sql`'s pattern for `products` — this is net-new category-document storage, not a data migration for the rename. Category *documents* live in the same table (`categories`) with their own `search_vector` column, not a separate table or a `doc_type` discriminator on a shared table — categories already have their own table, so extending it directly avoids the discriminator-column complexity for no benefit given there's exactly one non-product entity type.

`internal/infrastructure/postgres.SearchProductSource.ListAll`/`ListByIDs` (the read path both the reindex job and the event-driven subscriber ultimately go through, regardless of which `SearchEngine` is configured) now populate `CategoryIDs` via one batched query against `product_categories` per page — needed for Meilisearch's correctness, harmless overhead for Postgres (which ignores the field).

Meilisearch-backed deployments need a one-time full reindex after upgrading past this PR (existing documents keep whatever `category_id` they had; nothing auto-migrates a live index's document shape) — see `docs/guides/DEPLOYMENT.md`'s "Search Index Maintenance" section.

A category *document* (as opposed to its member products) is indexed via a direct `SearchEngine.IndexCategory`/`RemoveCategory` call from a new `IndexUpdateSubscriber.HandleCategoryCreated`/`Updated`/`Deleted`, not through `ReindexService`/the job queue — `ScopeCategories` only ever resolves to member *product* IDs (`ProductIDsByCategory`), never to the category document itself, so there was no existing scope/path this could reuse. A new narrow `domainsearch.CategorySource` port (mirroring `ProductSource`, implemented by `postgres.SearchCategorySource`) fetches fresh category data — including a live-computed `ProductCount` — keeping `search` decoupled from `catalog`, same as every other port in this track. Category events go through the same `bus.OnAsync` + direct call as products, but deliberately **without** PR-1036's debounce: category writes are single synchronous admin requests, not a bulk-import path, so there's no burst to coalesce.
...
| PR-1041 | Rate limiting hardening | **Done.** Today's `ratelimit.Limiter` was in-process only — correct for a single instance, silently wrong (each instance gets its own bucket) for horizontal scaling. Added a Redis-backed sliding-window limiter (`internal/infrastructure/redis.RateLimiter`, one atomic Lua script per check) as an opt-in alternative (`rate_limit.driver: memory\|redis`, default `memory` — zero new dependency for the common case), with its own independent Redis client (not a reuse of `cache.Cache`'s connection — see PR-1041.md's "Design decisions" for why that early plan didn't hold up). Documented TTL/window best practices: sliding window over fixed window (avoids the burst-at-boundary problem), a distinct budget for auth endpoints vs general API (auth lockout already did this correctly — the same reasoning is now documented for the general limiter's `rate_limit.per_route` config), and why probe/webhook endpoints need their own budget (already true for `/readyz`, which stays memory-only by design; payment webhooks and plugin integration routes were audited and don't need a separate mechanism — they already ride the general limiter). |
...
| B | 1.5–2 weeks | PR-1037's `CategoryIDs` change touches the Meilisearch-backed search index's document schema; a Meilisearch full reindex is required as part of that PR's rollout (Postgres-backed deployments are unaffected — see PR-1037's "Design notes") |
