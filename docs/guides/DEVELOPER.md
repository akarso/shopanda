...
- **Either way, this does NOT reach other replicas.** For data that actually mutates while the process runs (the category tree), the Store's own TTL is the real, only bound on cross-replica staleness. The permission catalog is the exception: `rbac.Registry` is frozen after plugin `Init()` and never mutates again, so the ONLY way its L1 entry can go stale is a new plugin version shipping a new/changed permission — and an old, un-restarted replica's TTL just keeps re-populating the SAME stale, pre-upgrade registry snapshot forever; waiting out the TTL there does not converge to the new value. Deployment/restart, not the TTL, is the actual bound for that case. The event only shortens same-process staleness to "however long dispatch takes" (typically sub-millisecond) — treat it as a nice-to-have for the process that made the change, not a distributed cache-coherence protocol.
