...
C4Component
    ...
    Container_Boundary(api, "API Server") {
        ...
        Boundary(application, "Application Layer (Use Cases)") {
            ...
            Component(reservationExpiryHandler, "ReservationExpiryHandler", "Go", "Handles inventory.reservation_expiry jobs: releases expired reservations in bounded batches, restores stock, publishes inventory.stock.updated")
            Component(indexUpdateSubscriber, "IndexUpdateSubscriber", "Go", "On-save search: listens for product/price/stock/category events, debounces, enqueues scoped ReindexService.Trigger")
        ...
        Boundary(infrastructure, "Infrastructure Layer (Adapters)") {
            ...
            Component(postgresSchedulerStore, "PostgresSchedulerStore", "Go, pgx", "Cross-process scheduler registrations + enable/disable overrides: implements scheduler.Store (consumed by CronScheduler) and scheduler.Catalog (consumed by ScheduleAdminHandler) over one table")
    ...
    Rel(cacheCleanupHandler, pgCacheStore, "Calls DeleteExpired (TTL rows + orphan cache_tags)")
    ...
    Rel(jobWorker, reservationExpiryHandler, "Dispatches inventory.reservation_expiry jobs")
    Rel(reservationExpiryHandler, postgresRepos, "ReleaseExpiredBefore: releases expired reservations, restores stock")
    Rel(reservationExpiryHandler, eventBus, "Publishes inventory.stock.updated per released reservation")
    Rel(eventBus, indexUpdateSubscriber, "inventory.stock.updated (and product/price/category events)")
    Rel(indexUpdateSubscriber, postgresJobQueue, "ReindexService.Trigger enqueues search.reindex")
    ...
    Rel(eventBus, localCacheStore, "Synchronous eviction on a subscribed domain event (catalog.EventCategory*)")
    ...
    Rel(pgCacheStore, postgres, "Key-value cache and cache_tags queries", "pgx")
    ...
    Rel(scheduleAdmin, postgresSchedulerStore, "List/Trigger/SetEnabled via scheduler.Catalog port")
    Rel(postgresSchedulerStore, postgres, "Reads/writes scheduler_tasks", "pgx")
