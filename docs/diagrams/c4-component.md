...
> **Wiring note (post PR-410–434, PR-1013, PR-1016, PR-1017, PR-1019):** `cmd/api/register_plugins.go` calls `core.Register()` for config-gated core plugins and optionally registers external plugins (e.g. `plugins/example`). A shared `Auditor` with `AuditLogRepo` is wired to admin mutation handlers. `PluginRegistry` calls `plugin.Init(pluginApp)` for each registered plugin. Before `InitAll`, serve wires an app-owned `rbac.Registry` on `pluginApp`; plugins register permissions into that instance; after `InitAll` the registry is frozen and serve `BindRuntime` so HTTP auth/catalog helpers share it (CLI/worker freeze only). Core plugins set infrastructure providers on `pluginApp` and may import `internal/infrastructure`; non-core plugins must not import `infrastructure`/`interfaces` (CI `TestImportBoundary`) and receive domain repos via `plugin.Bootstrap`. External plugins register pipeline steps, event handlers, permissions, and optional `RegisterConfig` settings. Serve composition root is explicit (no DI framework): `wire_repos.go` opens postgres repos, `wire_services.go` resolves providers and builds pipelines/workflows/handlers/event bus, `wire_routes.go` registers HTTP routes and wraps probes; `runServe` in `main.go` orchestrates DB open and server/worker/scheduler lifecycle. After HTTP drain, `runServe` stops admitting new `OnAsync` work, waits in-flight handlers with a live context for most of the 10s `Drain` budget (wait-then-cancel), then cancels stragglers; scheduler, worker, and bus drain are waited in parallel with 1s slack on `ShutdownBackground` so `event.bus.drain.timeout` can log before process exit.
>
