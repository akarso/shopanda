...
```go
app.Import().RegisterRowHook("product", 100, func(ctx *importctx.RowContext) error {
    // Map ERP column "MATNR" → slug, normalize VAT class, etc.
    return nil
})
```

Core importer calls the chain **after** header validation, **before** repository write. Transaction boundaries unchanged (one TX per batch or per row per existing importer).
