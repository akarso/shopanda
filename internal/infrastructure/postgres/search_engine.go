...
func (e *SearchEngine) Search(ctx context.Context, query search.SearchQuery) (search.SearchResult, error) {
	...
	default:
		if hasText {
			orderBy = fmt.Sprintf("ts_rank(p.search_vector, %s) DESC", tsQueryRef)
		}
	}
	...
	mainQ := fmt.Sprintf(
		"SELECT DISTINCT p.id, p.name, p.slug, p.description, p.attributes FROM products p %s WHERE %s ORDER BY %s LIMIT %s OFFSET %s",
		joinClause, whereClause, orderBy, limitArg, offsetArg)
