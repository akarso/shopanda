...
func (h *ShippingRatesHandler) List() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if currency == "" {
			httpshared.JSONError(w, apperror.Validation("currency is required"))
			return
		}

		itemCountStr := r.URL.Query().Get("item_count")
		if itemCountStr == "" {
			httpshared.JSONError(w, apperror.Validation("item_count is required"))
			return
		}
		itemCount, err := strconv.Atoi(itemCountStr)
		if err != nil || itemCount < 1 {
			httpshared.JSONError(w, apperror.Validation("item_count must be a positive integer"))
			return
		}
