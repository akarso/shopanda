...
func TestStorefrontHandler_ClaimRegister_Success(t *testing.T) {
	...
	h := storefront.NewStorefrontHandler(nil, nil, nil, nil, nil, nil).
...
func newStorefrontClaimPageHandler(t *testing.T, repo *storefrontOrderClaimRepoStub, linker *storefrontOrderLinkerStub) *storefront.StorefrontHandler {
	t.Helper()
	return storefront.NewStorefrontHandler(createTestTheme(t), nil, newStorefrontCategoryMock(), nil, nil, nil).
		WithOrderClaim(orderApp.NewClaimService(repo)).
		WithOrderLinker(linker).
		WithAccountSecurity("test-secret", time.Minute)
}

func storefrontClaimTokenForTest(t *testing.T, h *storefront.StorefrontHandler, repo *storefrontOrderClaimRepoStub, contactEmail string) string {
	t.Helper()
	emailer := &storefrontOrderClaimEmailerStub{}
