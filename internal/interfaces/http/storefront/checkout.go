...
func (h *CheckoutHandler) StartCheckout() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.CartID == "" {
			httpshared.JSONError(w, apperror.Validation("cart_id is required"))
			return
		}
		...
		if err != nil {
			httpshared.JSONError(w, apperror.Wrap(apperror.CodeInternal, "payable amount failed", err))
			return
		}
		...
		httpshared.JSON(w, http.StatusCreated, resp)
	}
}
