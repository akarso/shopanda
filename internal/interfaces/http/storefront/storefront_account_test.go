...
func TestStorefrontHandler_Home_ShowsAccountLoginEntry_WhenAnonymous(t *testing.T) {
	...
	h := storefront.NewStorefrontHandler(engine, &mockStorefrontRepo{}, newStorefrontCategoryMock(), pdp, plp, newStorefrontSearchMock())
...
func TestStorefrontHandler_Home_ShowsAccountControls_WhenAuthenticated(t *testing.T) {
	...
	h := storefront.NewStorefrontHandler(engine, &mockStorefrontRepo{}, newStorefrontCategoryMock(), pdp, plp, newStorefrontSearchMock()).WithAccount(authSvc, newStorefrontAccountOrderRepoStub(), &storefrontAccountDeleterStub{})
	router := newStorefrontRouter(h)
...
type storefrontAccountEmailTokenClaims struct {
	CustomerID string `json:"customer_id"`
	RedirectTo string `json:"redirect_to"`
	ExpiresAt  int64  `json:"expires_at"`
}

func assertStorefrontEmailVerificationEvent(t *testing.T, published []customer.EmailVerificationRequestedData, wantRedirect string) {
	t.Helper()
	if len(published) == 0 {
		t.Fatal("expected email verification event to be published")
	}
	verifyURL := published[len(published)-1].VerifyURL
	...
	if parsed.Path != "/account/verify-email" {
		t.Fatalf("verify path = %q, want %q", parsed.Path, "/account/verify-email")
	}
	...
	if claims.RedirectTo != wantRedirect {
		t.Fatalf("token redirect_to = %q, want %q", claims.RedirectTo, wantRedirect)
	}
}
...
func TestStorefrontHandler_AccountRegister_SendsEmailVerificationLink(t *testing.T) {
	engine := createTestTheme(t)
	pdp := composition.NewPipeline[composition.ProductContext]()
	plp := composition.NewPipeline[composition.ListingContext]()
	...
	if rec.Header().Get("Location") != "/account/verify-email?redirect_to=%2Faccount%2Forders&sent=1" {
		t.Fatalf("location = %q", rec.Header().Get("Location"))
	}
	if verifyURL == "" {
		t.Fatal("expected email verification event to publish a verify URL")
	}
	parsed, err := url.Parse(verifyURL)
	if err != nil {
		t.Fatalf("Parse verify URL: %v", err)
	...
	if parsed.Query().Get("email_token") == "" {
		t.Fatal("expected email_token query parameter")
	}
}

func TestStorefrontHandler_AccountVerifyEmail_MarksCustomerVerified(t *testing.T) {
	engine := createTestTheme(t)
	pdp := composition.NewPipeline[composition.ProductContext]()
	plp := composition.NewPipeline[composition.ListingContext]()
	bus := event.NewBus(logger.New("error"))
	...
	if registered == nil || registered.EmailVerifiedAt == nil {
		t.Fatal("expected registered customer email to be marked verified")
	}
	if !strings.Contains(verifyRec.Body.String(), "Your email address is verified.") {
		t.Fatalf("expected verification success in body: %s", verifyRec.Body.String())
	}
}
...
func TestStorefrontHandler_AccountOrders_RendersCustomerOrders(t *testing.T) {
	...
	h := storefront.NewStorefrontHandler(engine, &mockStorefrontRepo{}, newStorefrontCategoryMock(), pdp, plp, newStorefrontSearchMock()).WithAccount(authSvc, orders, &storefrontAccountDeleterStub{})
...
func TestStorefrontHandler_AccountSecurity_RedirectsToEmailVerification_WhenEmailUnverified(t *testing.T) {
	...
	assertStorefrontEmailVerificationEvent(t, published, "/account/security")
}
...
func TestStorefrontHandler_AccountPassword_RedirectsToEmailVerification_WhenEmailUnverified(t *testing.T) {
	engine := createTestTheme(t)
	pdp := composition.NewPipeline[composition.ProductContext]()
	plp := composition.NewPipeline[composition.ListingContext]()
	authSvc, _ := newStorefrontAuthService(t)
	...
	if rec.Header().Get("Location") != "/account/verify-email?redirect_to=%2Faccount%2Fsecurity&sent=1" {
		t.Fatalf("location = %q, want %q", rec.Header().Get("Location"), "/account/verify-email?redirect_to=%2Faccount%2Fsecurity&sent=1")
	}
}
...
func TestStorefrontHandler_AccountDelete_RequiresConfirmation(t *testing.T) {
	...
	h := storefront.NewStorefrontHandler(engine, &mockStorefrontRepo{}, newStorefrontCategoryMock(), pdp, plp, newStorefrontSearchMock()).WithAccount(authSvc, newStorefrontAccountOrderRepoStub(), deleter).WithAccountSecurity("test-secret", time.Minute).WithAccountSecurityEmailLinks("https://shop.test", 45*time.Minute)
	router := newStorefrontRouter(h)
	id, err := identity.NewIdentity(out.CustomerID, identity.RoleCustomer)
	if err != nil {
		t.Fatalf("NewIdentity: %v", err)
