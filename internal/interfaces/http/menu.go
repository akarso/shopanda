...
func (h *MenuHandler) GetByCode() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if code == "" {
			JSONError(w, apperror.Validation("menu code is required"))
			return
		}
		if !cms.ValidMenuCode(code) {
			JSONError(w, apperror.Validation("invalid menu code"))
			return
		}
		...
		if data == nil || !data.Menu.IsActive() {
			JSONError(w, apperror.NotFound("menu not found"))
			return
		}
		...
		JSON(w, http.StatusOK, publicMenuResponse{
			Code:  data.Menu.Code(),
