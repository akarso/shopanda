	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := h.exporter.Export(r.Context(), &buf, exporter.OssExportOptions{
			...
			if errors.Is(err, legal.ErrOssExportDisabled) {
				httpshared.JSONError(w, apperror.Forbidden("OSS export is disabled for this store"))
				return
			}
			httpshared.JSONError(w, apperror.Wrap(apperror.CodeInternal, "oss export failed", err))
			return
		}
