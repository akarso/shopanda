...
func (h *ShippingZoneAdminHandler) CreateZone() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.FreeShippingThreshold != nil && *req.FreeShippingThreshold != 0 {
			...
			if m.Amount() < 0 {
				httpshared.JSONError(w, apperror.Validation("free_shipping_threshold must be >= 0"))
				return
			}
...
func (h *ShippingZoneAdminHandler) UpdateZone() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		zid := r.PathValue("id")
		if zid == "" {
			httpshared.JSONError(w, apperror.Validation("zone id is required"))
			return
		}
		...
		if zone == nil {
			httpshared.JSONError(w, apperror.NotFound("shipping zone not found"))
			return
		}

		if req.Name != nil {
			if *req.Name == "" {
				httpshared.JSONError(w, apperror.Validation("name must not be empty"))
				return
			}
			zone.Name = *req.Name
		}
		if req.Countries != nil {
			if len(req.Countries) == 0 {
				httpshared.JSONError(w, apperror.Validation("countries must not be empty"))
				return
			}
			for _, c := range req.Countries {
				if len(c) != 2 {
					httpshared.JSONError(w, apperror.Validation("country code must be 2 characters: "+c))
					return
				}
			}
...
func (h *ShippingZoneAdminHandler) UpdateRate() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if zoneID == "" || rateID == "" {
			httpshared.JSONError(w, apperror.Validation("zone id and rate id are required"))
			return
		}
		...
		if tier == nil {
			httpshared.JSONError(w, apperror.NotFound("rate tier not found"))
			return
		}
		if tier.ZoneID != zoneID {
			httpshared.JSONError(w, apperror.NotFound("rate tier not found in this zone"))
			return
		}
