...
func (h *ContentBlockAdminHandler) Get() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if blockID == "" {
			httpshared.JSONError(w, apperror.Validation("content block id is required"))
			return
		}
		...
		if block == nil {
			...
			httpshared.JSONError(w, apperror.NotFound("content block not found"))
			return
		}
		h.audit(r, admin.AuditContentBlockRead, blockID, map[string]interface{}{"block_type": block.BlockType()}, nil)
		httpshared.JSON(w, http.StatusOK, toAdminContentBlockResponse(block))
	}
}
...
func (h *ContentBlockAdminHandler) Create() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		httpshared.JSON(w, http.StatusCreated, toAdminContentBlockResponse(block))
	}
}
...
func (h *ContentBlockAdminHandler) Delete() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		httpshared.JSON(w, http.StatusOK, map[string]string{"status": "deleted"})
	}
}
