...
type mockStatsRepo struct {
	getDashboardStatsFn func(ctx context.Context, lowStockThreshold, recentLimit int) (domainadmin.DashboardStats, error)
}

func (m *mockStatsRepo) GetDashboardStats(ctx context.Context, lowStockThreshold, recentLimit int) (domainadmin.DashboardStats, error) {
...
func TestStatsAdminHandler_Overview_AuditIncludesScopeContext(t *testing.T) {
	repo := &mockStatsRepo{
		getDashboardStatsFn: func(_ context.Context, lowStockThreshold, recentLimit int) (domainadmin.DashboardStats, error) {
			...
			return domainadmin.DashboardStats{
				OrdersToday:   3,
				RevenueToday:  4200,
				Currency:      "EUR",
				TotalProducts: 15,
				LowStockCount: 2,
				RecentOrders: []domainadmin.RecentOrder{{
					ID:          "ord-1",
					CustomerID:  "cust-1",
					TotalAmount: 4200,
					Currency:    "EUR",
	...
	sink := &auditSink{}
	h := admin.NewStatsAdminHandlerWithAuditor(repo, adminapp.NewAuditor(sink))
...
func TestStatsAdminHandler_Overview_AuditFailureIncludesError(t *testing.T) {
	repo := &mockStatsRepo{
		getDashboardStatsFn: func(_ context.Context, lowStockThreshold, recentLimit int) (domainadmin.DashboardStats, error) {
			return domainadmin.DashboardStats{}, errors.New("stats unavailable")
		},
	}
	sink := &auditSink{}
	h := admin.NewStatsAdminHandlerWithAuditor(repo, adminapp.NewAuditor(sink))
...
func TestStatsAdminHandler_Overview_CustomerForbidden(t *testing.T) {
	repo := &mockStatsRepo{}
	h := admin.NewStatsAdminHandlerWithAuditor(repo, adminapp.NewAuditor(logger.NewWithWriter(io.Discard, "info")))
