...
func (h *SchemaHandler) GetForm() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if name == "" {
			httpshared.JSONError(w, apperror.Validation("form name is required"))
			return
		}

		form, ok := h.registry.Form(name)
		if !ok {
			httpshared.JSONError(w, apperror.NotFound("form not found"))
			return
		}
...
func (h *SchemaHandler) GetGrid() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		name := r.PathValue("name")
		if name == "" {
			httpshared.JSONError(w, apperror.Validation("grid name is required"))
			return
		}

		grid, ok := h.registry.Grid(name)
		if !ok {
			httpshared.JSONError(w, apperror.NotFound("grid not found"))
			return
		}
