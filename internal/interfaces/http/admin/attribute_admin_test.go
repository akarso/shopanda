...
func newAttributeAdminRouter(h *admin.AttributeAdminHandler) *http.ServeMux {
...
func TestAttributeAdminHandler_CreateSyncsDiscoveryFacets(t *testing.T) {
	...
	h := admin.NewAttributeAdminHandler(store).WithDiscoveryFacetSync(syncer)
	router := newAttributeAdminRouter(h)
...
func TestAttributeAdminHandler_DeleteNotFoundRetriesFacetSync(t *testing.T) {
	...
	h := admin.NewAttributeAdminHandler(store).WithDiscoveryFacetSync(failing)
	router := newAttributeAdminRouter(h)
	...
	h = admin.NewAttributeAdminHandler(store).WithDiscoveryFacetSync(recording)
	router = newAttributeAdminRouter(h)
...
func TestSchemaHandler_ProductFormIncludesAttributes(t *testing.T) {
	...
	handler := admin.NewSchemaHandler(registry, store)
	mux := http.NewServeMux()
