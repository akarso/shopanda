...
func (h *ConfigAdminHandler) Get() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		httpshared.JSON(w, http.StatusOK, payload)
	}
}
...
func (h *ConfigAdminHandler) TestEmail() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.TrimSpace(req.To) == "" {
			httpshared.JSONError(w, apperror.Validation("to is required"))
			return
		}
		...
		if err := h.testEmailFunc(r.Context(), settings, strings.TrimSpace(req.To)); err != nil {
			httpshared.JSONError(w, apperror.Internal("failed to send test email: "+err.Error()))
			return
		}
