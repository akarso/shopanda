...
func (h *InvoiceAdminHandler) ListByOrder() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			httpshared.JSONError(w, apperror.Wrap(apperror.CodeInternal, "list order invoices failed", err))
			return
		}
		if ord == nil {
...
func (h *InvoiceAdminHandler) DownloadPDF() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if invoiceID == "" {
			httpshared.JSONError(w, apperror.Validation("invoice id is required"))
			return
		}

		inv, err := h.invoices.FindByID(r.Context(), invoiceID)
		if err != nil {
			httpshared.JSONError(w, apperror.Wrap(apperror.CodeInternal, "download invoice pdf failed", err))
			return
		}
		if inv == nil {
			httpshared.JSONError(w, apperror.NotFound("invoice not found"))
			return
		}
