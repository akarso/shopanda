...
// CatalogEntry describes registered import row hooks for an entity.
...
// Registry stores import row hook handlers and executes ordered chains per entity.
// Panics in handlers are recovered and returned as errors.
...
// NewRegistry creates an empty import row hook registry.
...
func (r *Registry) Register(entity string, priority int, registrant string, handler RowHandler) error {
	if r == nil {
		return fmt.Errorf("importctx: registry must not be nil")
	}
	...
	if registrant == "" {
		return fmt.Errorf("importctx: registrant must not be empty")
	}
	if handler == nil {
		return fmt.Errorf("importctx: handler must not be nil")
	}
...
func (r *Registry) runHandler(rowCtx *RowContext, h registeredHandler) (err error) {
	defer func() {
		if rec := recover(); rec != nil {
			if r.log != nil {
				r.log.Error("importctx.handler.panic", fmt.Errorf("%v", rec), map[string]interface{}{
					"entity":     rowCtx.Entity,
					"hook":       h.hook,
					"registrant": h.registrant,
				})
			}
			err = fmt.Errorf("import row hook %q handler %q panicked: %v", h.hook, h.registrant, rec)
		}
	}()
	return h.handler(rowCtx)
}

// Catalog returns registered import row hooks grouped by entity.
