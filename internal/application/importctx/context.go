...
// RowHandler mutates import row context during synchronous chain execution.
type RowHandler func(ctx *RowContext) error

// RowContext carries mutable CSV row data across import row hook handlers.
...
func NewRowContext(entity string, rowIndex int, row map[string]string) (*RowContext, error) {
	...
	if rowIndex <= 0 {
		return nil, fmt.Errorf("import row index must be greater than zero")
	}
