...
func buildServeHandler(cfg *config.Config, log logger.Logger, rt *serveRuntime, conn *sql.DB) (http.Handler, error) {
	...
	// duration/span, including every other middleware.
	router.Use(shophttp.MetricsMiddleware(rt.metricsRecorder))
	router.Use(shophttp.TracingMiddleware())
	...
	router.Handle("POST /api/v1/admin/schedules/{name}/disable", requireJobsWrite(rt.scheduleAdmin.Disable()))
